...
export const PersonalInfo: React.FC<PersonalInfoProps> = ({
  ...
  const [showConnect, setShowConnect] = useState(personalInfo.location === 'desconocida');
        ...
        <Select onChange={(e) => setPerformerConnect(e.target.value)}>
          <option>Modelos:</option>
          {
            performers.sort((a: Performer, b: Performer) => {
              ...
              return <option key={index} value={performer._id} >{performer.performerName}</option>;
            })
          }
        </Select>
        <FcCancel onClick={() => setShowPerformers(false)} />
        <FcOk onClick={handleConnect}/>
      </PerformerConnect>      
    }
  </Wrapper>
  );
